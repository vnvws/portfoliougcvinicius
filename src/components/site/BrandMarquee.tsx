...
function Logo({ name, src }: Brand) {
  return (
    <div
      ...
      className="group relative mx-3 sm:mx-4 flex h-14 w-14 sm:h-20 sm:w-20 lg:h-24 lg:w-24 shrink-0 items-center justify-center overflow-hidden rounded-2xl transition-all duration-300 hover:scale-[1.05]"
      style={{ 
...
export function BrandMarquee() {
  return (
    <div className="group relative overflow-hidden py-6 lg:py-10 touch-pan-x" style={{ isolation: 'isolate' }} role="list" aria-label="Marcas que já confiaram">
      ...
      <div
        className="pointer-events-none absolute inset-y-0 left-0 w-16 sm:w-40 z-10"
        style={{ background: "linear-gradient(to right, var(--color-bone), transparent)" }}
      />
      <div
        className="pointer-events-none absolute inset-y-0 right-0 w-16 sm:w-40 z-10"
